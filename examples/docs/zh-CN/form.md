...
:::tip
当在多语言环境时，表单验证的错误信息是不能跟着切换的，这时候需要些黑科技（随着版本迭代，这黑科技需要跟着变更）
```js
  // 定义一个公共方法
  export function toggleLangValidateErrMsg (vm, form) {
    if (vm.$refs[form]) {
      vm.$refs[form].fields.forEach((item) => {
        if (item.validateMessage !== '') {
          vm.$refs[form].validateField(item.prop)
        }
      })
    }
  }

  // 实际使用场景中，监听lang，然后调用那个公共方法
  watch: {
    'lang' (val) {
      toggleLangValidateErrMsg(this, curFormObj);
    }
  }
```
:::
