  ...
  export default {
    ...
    methods: {
      ...
      querySearchAsync(queryString, cb) {
        this.filterWords = queryString
        this.fetchData(queryString, cb)
      },
      fetchData (queryString, cb) {
        var restaurants = this.restaurants;
        var results = queryString ? restaurants.filter(this.createStateFilter(queryString)) : restaurants;
        this.showLoadMore = results.length > (this.pageOffset + 1) * this.pageSize
        let res = results.slice(0, (this.pageOffset + 1) * this.pageSize)
      ...
      loadMoreEvent (cb) {
        this.pageOffset += 1
        this.fetchData(this.filterWords, cb)
      }
    },
    mounted() {
...
| prefix-icon | 输入框头部图标 | string | — | — |
| suffix-icon | 输入框尾部图标 | string | — | — |
| show-load-more | 展示加载更多按钮 | boolean | [true, false] | false |
| load-more-text | 加载更多文案更改 | string | - | '加载更多' |
...
| loadMore | 点击加载更多时触发, 通过调用 callback(data:[]) 插入更多数据 | Function(callback) |
